...
source(fs::path(rdir, "FRS_funding_amort.R"), local = fm_env) # only creates amort function - no live code
source(fs::path(rdir, "FRS_funding_model_functions_loop_try.R"), local = fm_env) # only creates loop function - no live code
source(fs::path(rdir, "FRS_get_funding_function_only.R"), local = fm_env) # only runs the get funding function 

...
build_drop_drivers <- function(funding_list, frs_fund) {
  ...
  for (i in 2:n) {
    ...
    # Alternatively, using regular to calculate NC rates
    # out$nc_rate_db_legacy[i] <- if (is.na(reg$payroll_db_legacy[i]) || reg$payroll_db_legacy[i] == 0) 0
    # else reg$nc_legacy[i] / reg$payroll_db_legacy[i]
    # 
    # out$nc_rate_db_new[i]    <- if (is.na(reg$payroll_db_new[i]) || reg$payroll_db_new[i] == 0) 0
    # else reg$nc_new[i] / reg$payroll_db_new[i]
...
main_loop <- function(funding_list,
  ...
  for (i in 2:nrow(funding_list[[1]])) { # loop 2nd year to last year
    ...
    funding_list <- inner_drop1_funding_cached(i,
                                        funding_list,
                                        drop_drivers,

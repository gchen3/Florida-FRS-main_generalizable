...
get_wf_active_df_final_s <- function(wf_active_df,
                                   benefit_val_table,
                                   params){
  #Join wf active table with FinalData table to calculate the overall payroll, normal costs, PVFB, and PVFS each year
  wf_active_df_final_s <- wf_active_df %>% 
    filter(year <= params$start_year_ + params$model_period_) %>% 
    mutate(entry_year = year - (age - entry_age)) %>% 
    left_join(benefit_val_table, by = c("entry_age", "age" = "term_age", "year" = "term_year", "entry_year")) %>% 
    select(entry_age, age, year, entry_year, n_active, indv_norm_cost, salary, 
           pvfb_db_wealth_at_current_age, pvfnc_db, pvfs_at_current_age) %>% 
    left_join(db_dc_legacy_table %>% filter(class == class_name) %>% select(-class), 
              by = join_by(entry_year >= year_ll, 
                           entry_year < year_ul)) %>%
    left_join(db_dc_new_table %>% filter(class == class_name) %>% select(-class),
              by = join_by(entry_year >= new_year_ll,
...
wf_active_df_final_s <-  get_wf_active_df_final_s(wf_active_df,
                                    benefit_val_table,
                                    params)
...
#TRUE
identical(wf_active_df_final, wf_active_df_final_s)
...
wf_term_df_final_s <- get_wf_term_df_final_s(wf_term_df,
                        benefit_val_table,
                        benefit_table,
                        params)
...
identical(wf_term_df_final, wf_term_df_final_s) #TRUE
...
get_wf_refund_df_final_s <- function(wf_refund_df,
                                   benefit_table,
                                   params){
  # Join wf refund table with benefit table to calculate the overall refunds each year
  wf_refund_df_final_s <- wf_refund_df %>% 
    filter(year <= params$start_year_ + params$model_period_,
           n_refund > 0) %>% 
    mutate(entry_year = year - (age - entry_age)) %>% 
    left_join(benefit_table, 
...
identical(
  get_wf_refund_df_final_s(wf_refund_df,
                           benefit_table,
                           params),
...
get_wf_retire_df_final_s <- function(wf_retire_df,
                                   benefit_table,
                                   ann_factor_table,
                                   params){
  # Join wf retire table with benefit table to calculate the overall retirement benefits each year
  wf_retire_df_final_s <- wf_retire_df %>% 
    filter(year <= params$start_year_ + params$model_period_) %>% 
    mutate(entry_year = year - (age - entry_age)) %>%    
    left_join(benefit_table, by = c("entry_age", "entry_year", "term_year", "retire_year" = "dist_year")) %>% 
    select(entry_age, age, year, term_year, retire_year, entry_year, n_retire, db_benefit, cola) %>% 
...
identical(
  get_wf_retire_df_final_s(wf_retire_df,
                           benefit_table,
                           ann_factor_table,
                           params),
...
get_liability_data_s <- function(
  ...
  funding_df <- get_funding_df(wf_active_df_final_s,
                               wf_term_df_final_s,
...
identical(liability_data_regular, liability_data_s) #TRUE
...
# Check if the two functions return the same result
identical(liability_data_special, liability_data_s) #TRUE
